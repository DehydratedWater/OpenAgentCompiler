"""Render individual workflow steps for pi agent prompts.
...
- Tool usage notes
- Subagent references (using pi's Agent() tool syntax)
...
def render_step_for_pi(step: WorkflowStepDefinition, agent: AgentVariant) -> str:
    ...
    if step.subagents:
        ...
        for sub_name in step.subagents:
            ...
            if sub_def:
                parts.append(
                    f'- Spawn `{sub_name}` via `Agent({{ "subagent_type": "{sub_name}",'
                    f' "prompt": "<task description>", "description": "{sub_def.description or sub_name}" }})`'
                )
            else:
